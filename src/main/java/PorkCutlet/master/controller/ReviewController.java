...
import PorkCutlet.master.controller.dto.ReviewForm;
...
import PorkCutlet.master.service.ImageService;
...
public class ReviewController {
	...
	public String deleteReview(@Login UserInfoDto user, @PathVariable Long reviewId, Model model) {
		...
		imageUtils.deleteImageFilesByStoreFileName(
			review.getImages().stream().map(Image::getStoreImageName).collect(Collectors.toList()));
	...
	public Object updateReview(@Login UserInfoDto user, @PathVariable Long reviewId, @Valid ReviewForm reviewForm,
		...
		reviewService.updateReview(reviewId, reviewForm.getForkCutletName(), reviewForm.makeForkCutletType(),
			reviewForm.getRestaurantName(),
			reviewForm.makeAddress(), reviewForm.getContent(), reviewForm.getOneSentence(), reviewForm.makeRatingInfo(),
			updatedImages);
		return "{\"result\":\"OK\"}";
	}

	private List<Image> getUpdatedImages(List<MultipartFile> files, List<String> deleteImages, Long reviewId) throws
		IOException {
	...
	private void validateFile(BindingResult bindingResult, List<MultipartFile> files, List<String> preImages,
	...
	private Review makeReview(ReviewForm reviewForm, List<Image> images, User loginUser) {
		Review review = new Review(loginUser, reviewForm.makeRestaurant(), images, reviewForm.getContent(),
			reviewForm.getOneSentence(), reviewForm.makeRatingInfo());
		return review;
	}
