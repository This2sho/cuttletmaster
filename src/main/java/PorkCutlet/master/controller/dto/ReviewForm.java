...
public class ReviewForm {
    ...
    public static ReviewForm from(Review review) {
        return new ReviewForm(review.getRestaurant().getName(), review.getRestaurant().getForkCutlet().getName(), review.getRestaurant().getForkCutlet().getForkCutletType().getValue(),
                review.getRestaurant().getAddress().getRoadAddress(), review.getRestaurant().getAddress().getDetailAddress(), review.getOneSentence(), review.getContent(),
                review.getRatingInfo().getTexture().getValue(), review.getRatingInfo().getSauce().getValue(), review.getRatingInfo().getSide().getValue(), review.getRatingInfo().getAccessibility().getValue(), review.getImages().stream().map(Image::getStoreImageName).collect(Collectors.toList()),new ArrayList<>(), null);
    }
}
