...
public class CommentController {
	...
	public String getComments(@Login UserInfoDto user, @PathVariable Long reviewId, Model model,
		...
		String reviewCreator =
			commentsWithFetchJoin.size() > 0 ? commentsWithFetchJoin.get(0).getReview().getUser().getNickName() : "";
	...
	public String updateComment(@Login UserInfoDto user, @PathVariable Long reviewId, @PathVariable Long commentId,
		String content, int pageNum, Model model) throws AccessException {
		if (user == null) {
			return "CONST-ERROR";
		}
	...
	public String deleteComment(@Login UserInfoDto user, @PathVariable Long reviewId, @PathVariable Long commentId,
		int pageNum, Model model) throws AccessException {
		if (user == null) {
			return "CONST-ERROR";
		}
